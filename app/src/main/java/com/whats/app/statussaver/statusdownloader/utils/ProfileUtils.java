...
public class ProfileUtils {
    ...
    public String getDeviceOwnerName() {
        ...
        if (CommonUtils.isSupport(14)) {
            ...
            if (c != null && c.getCount() > 0 && c.moveToFirst()) {
                @SuppressLint("Range") final String ownerName = c.getString(c.getColumnIndex(ContactsContract.Profile.DISPLAY_NAME));
                c.close();
                return ownerName;
            }
    ...
    private String getAccountName(final Pattern pattern) {
        @SuppressLint("MissingPermission") final Account[] accounts = AccountManager.get(context).getAccounts();
